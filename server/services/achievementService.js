...
function updateAchievements(userId, callback) {
  db.get('SELECT * FROM users WHERE id = ?', [userId], (errUser, user) => {
    ...
    db.get('SELECT * FROM user_mastery WHERE user_id = ?', [userId], (errMastery, mastery) => {
      ...
      const masteryFunctions = mastery ? mastery.functions_correct || 0 : 0;
      const masterySequences = mastery ? mastery.sequences_correct || 0 : 0;
      const masteryEquations = mastery ? mastery.equations_correct || 0 : 0;
      const masteryRates = mastery ? mastery.rates_correct || 0 : 0;
      const masteryFactors = mastery ? mastery.factors_correct || 0 : 0;
          ...
          db.get('SELECT COUNT(*) AS count FROM user_inventory WHERE user_id = ?', [userId], (errInv, rowInv) => {
            ...
            db.all('SELECT * FROM achievements', (errAchs, achs) => {
              ...
              achs.forEach((ach) => {
                ...
                else if (type === 'mastery_functions') progress = masteryFunctions;
                else if (type === 'mastery_sequences') progress = masterySequences;
                else if (type === 'mastery_equations') progress = masteryEquations;
                else if (type === 'mastery_rates') progress = masteryRates;
                else if (type === 'mastery_factors') progress = masteryFactors;
